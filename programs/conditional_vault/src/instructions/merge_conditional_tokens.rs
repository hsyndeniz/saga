...
impl InteractWithVault<'_> {
  ...
  pub fn handle_merge_conditional_tokens(ctx: Context<Self>, amount: u64) -> Result<()> {
    ...
    let pre_user_conditional_on_finalize_balance = ctx.accounts.user_conditional_on_finalize_token_account.amount;
    let pre_user_conditional_on_revert_balance = ctx.accounts.user_conditional_on_revert_token_account.amount;
